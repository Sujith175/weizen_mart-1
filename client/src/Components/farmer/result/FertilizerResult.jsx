...
import { output_descriptions, label_image_paths } from '../fertilizers/FertilizerOutputs';
...
export function FertilizerResult() {
    ...
    useEffect(() => {
        if (locationState == null) {
            console.log("Redirecting to /fertilizers...")
            navigate("/farmer/fertilizers")
        }
    ...
    return (
        <>
            ...
            <img className="fertilizer-result-img" src={output_image_path} alt="Loading" />
            <p className="fertilizer-result-description"> {output_descriptions[predicted_fertilizer]} </p>
            <button className="fertilizer-try-btn" onClick={() => navigate("/farmer/fertilizers")}> Try again ? </button>
        </>
    );
}
